...
app.get("/exit", function (request, response) {
    ...
    console.log("exit");
     //response.sendFile(path.join(__dirname + '/indexNoLogin.html'));
...
app.post("/buyVip",async function (request, response) {
      if(!request.body) return response.sendStatus(400);
      if(request.session.loggedin){
        let login=request.session.login;
        console.login("buy VIP",login);
        await User.updateOne({ login : login }, {
            vip:true
        }); 
      }
      response.redirect('/');
...
app.post("/task",async function (request, response) {
    ...
    task.save(function(err){if(err) return console.log(err);});
     /*
    let ifIsfreeServer=false;
     for (var i = 0; i < freeServers.length; i++) {
     	if (freeServers[i]==true){
        Request.post({url: 'http://127.0.0.1:800'+String(i+1)+'/solve?taskId='+String(task._id)}, (err, res, body) => {
     ...
     */
     	...
     	if (freeServers[0]==true){
     		...
     		queueTasks.push(task._id);
     		console.log(queueTasks);
    		//console.log("----boolls: ",freeServers[0],", ",freeServers[1],", ",freeServers[2]);
     	}
